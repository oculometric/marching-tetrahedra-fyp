...
\usepackage[a4paper, portrait, margin=0.8in, top=0.5in, bottom=0.5in]{geometry}
...
\urlstyle{same}
\hypersetup{
pdftitle={An Examination of Isosurface Extraction for 3D Terrain Generation in Videogames},
pdfauthor={Cassette Costen},
 hidelinks, pdfcreator={cassette :)}
pdfkeywords={Important Keywords},%
}

\title{\textbf{An Examination of Isosurface Extraction for 3D Terrain Generation in Videogames}}
\author{Cassette Costen - Supervised by Kieran Osborne (University of Staffordshire)}
\date{March 2026}
...
\begin{center}
    \includegraphics[width=0.75\textwidth]{header.png}
\end{center}

\FloatBarrier

\begin{abstract}
Procedural environment generation for videogames sometimes demands a more advanced implementation than simple heightmaps, in the form of unique topologies. The creation of such environments presents unique challenges, in that the resulting mesh must be watertight and highly regular without human supervision, while at the same time being computationally cheap to generate. This paper compares several approaches to extracting isosurfaces as triangle meshes, and examines requirements and opportunities for further development of mesh generation algorithms specialised to suit this use case.
\end{abstract}

\bigskip

\keywords{isosurface extraction, marching tetrahedra, computer graphics, video games, procedural generation, triangular mesh, regularisation}

\newpage

\section*{Acknowledgements}
\emph{I want to thank \textbf{Kieran Osborne} for being a fantastic supervisor throughout this project. I also want to thank \textbf{Lucy} and \textbf{Louise} for looking out for me and listening to me excitedly ramble, as well as all of my housemates, friends, and my parents --- Thankyou.}
...
Procedural mesh generation presents several challenges, particularly for use in videogames. Resulting meshes must be visually complete and water-tight without additional human intervention, must be minimal in triangle count to meet real-time performance targets, and must produce sensible results when shaded with common techniques.

The mesh generation itself must be highly optimised for both performance and memory consumption - with particular consideration given to minimising startup time.

Literature in relation to relevant isosurface extraction methods is examined in this report, and the landmark \textquotesingle regularised marching tetrahedra\textquotesingle{} algorithm presented by Treece et. al. (1999) is implemented.

An alternative arrangement of the tetrahedral lattice, and an alternative mesh simplification algorithm are demonstrated and the impact of these variations is examined with regard to the computation time, triangle count and mean aspect ratio, and perceived visual quality of resulting meshes

This paper aims to detail the state of isosurface extraction techniques and explore further opportunities specifically regarding use in videogame terrain generation.

\subsection{Technical Considerations}\label{technical-considerations}

For real-time rendering applications such as videogames, triangle counts should be reduced where possible to minimise GPU utilisation. Since terrain meshes often take on organic forms as a result of procedural noise, some even being grown using evolutionary algorithms (Raffe et al., 2012), they can benefit greatly from regularisation techniques which are able to distribute triangles and eliminate small faces. Extremely small or narrow faces which often result from isosurface extraction algorithms are wasteful both due to overall triangle count, and as a result of overdraw.

Nearly all modern graphics acceleration hardware shade pixels in $2\times2$ quads to facilitate calculation of derivatives of per-pixel variables such as UV coordinates (Penner and Engel, 2011). Overdraw occurs when wasted work is performed due to a triangle only covering a portion of a quad, resulting in pixels being shaded many more times than is necessary and contributing to worse performance. This scenario becomes very frequent when rendering meshes with many highly elongated or tiny triangles.
...
\begin{figure}[htb]
    \centering
    \includegraphics[width=0.6\textwidth]{evil triangles.png}
...
\begin{figure}[htb]
    \centering
    \includegraphics[width=0.6\textwidth]{nice triangles.png}
...
Ideally, all triangles in the resulting mesh should be equilateral, or as close to equilateral as possible to tile the surface, as this minimises the deviation in internal corner angles for the triangle and thus also minimises the appearance of shading anomalies. Tiling a surface with equilateral triangles also results in the most efficient filling of surface area, which significantly reduces work wasted due to overdraw. Within this paper, triangles being described as poor quality implies they are highly elongated or extremely small.
...
\begin{singlecolfigure}
    \centering
    \includesvg[inkscapelatex=false,width=0.85\textwidth]{circle 1d manifold.svg}
    \captionof{figure}{A 1-manifold representing the isosurface defined by $f(x, y) = \sqrt{x^2 + y^2}$ and isovalue 1.}
...
\begin{singlecolfigure}
    \centering
    \includegraphics[width=0.85\textwidth]{marching cubes configurations.png}
    \captionof{figure}{The 14 unique triangulations used by the marching cubes algorithm (Shu et al., 1995).}
    \label{fig:mcconfigs}
\end{singlecolfigure}
...
\begin{singlecolfigure}
    ...
    \captionof{figure}{Illustration of the ambiguity problem with marching cubes. Without additional information, it cannot be determined which of the two triangulations is appropriate, which results in holes between cubes when extended into 3D.}
...
\begin{singlecolfigure}
    ...
    \captionof{figure}{The unique configurations for a tetrahedral cell. Note that the fourth case shown is technically an inversion of the second case, meaning that only three unique cases exist, only two of which contain geometry.}
    \label{fig:mtconfigs}
\end{singlecolfigure}
...
\begin{singlecolfigure}
    \centering
    \includegraphics[width=0.8\textwidth]{elastic surface nets.jpg}
    \captionof{figure}{The elastic surface nets algorithm demonstrated in 2D. From left to right, each image shows a greater degree of relaxation (Gibson, 1998).}
...
\begin{singlecolfigure}
    \centering
    \includesvg[inkscapelatex=false,width=0.9\textwidth]{tetrahedron subdivision.svg}
    \captionof{figure}{a) regular subdivision of a tetrahedra cell; b) four possible irregular subdivisions.}
    \label{fig:tetsub}
\end{singlecolfigure}

\begin{singlecolfigure}
    \centering
    \includegraphics[width=0.5\textwidth]{inconsistent tessellation density.png}
    \captionof{figure}{Illustration of inconsistent surface tessellation density (Bey, 1995).}
...
\begin{singlecolfigure}
    ...
    \captionof{figure}{Example of a local re-triangulation to eliminate the vertex R (Turk, 1992).}
...
\begin{figure}[htb]
    \centering
    \includesvg[width=0.75\textwidth]{candidate vertices.svg}
...
\begin{figure}[htb]
    ...
    \caption{Example transformation from spatial representation into graph representation. As described, only nearby candidates participate (labelled $A\dots E$), and only adjacent candidates may be connected together. This results in the graph on the right, where it can be seen that the topology consists of two separate islands (labelled $I_1$ and $I_2$).}
...
\subsubsection{Optimisation Strategies}\label{optimisation-strategies}
...
Examples of algorithm output, displayed in the testing environment, are shown below. As can be seen in Fig. \ref{fig:imagesout2} and \ref{fig:imagesout3}, the algorithm produces appropriate results for both smooth and flat mesh shading scenarios; the latter is comparable to the appearance of the polygonal terrain in ASTRONEER (2016).
...
\begin{subfigure}{.33\textwidth}
  \centering
  \includegraphics[width=.9\linewidth]{as_bi.png}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
  \centering
  \includegraphics[width=.9\linewidth]{af_bi.png}
\end{subfigure}%
\begin{subfigure}{.33\textwidth}
  \centering
  \includegraphics[width=.9\linewidth]{as_biw.png}
\end{subfigure}
\caption{Images of mesh output for the asteroid with a medium sample resolution, using a body-centered arrangement with the integrated clustering algorithm. Flat shading is shown in the center, with the wireframe shown on the right.}
\label{fig:imagesout3}
\end{figure}
...
\begin{axis}[
	...
	axis equal=false,
	minor tick num=1,
	yticklabel=\pgfkeys{/pgf/number format/.cd,fixed,precision=3,zerofill}\pgfmathprintnumber{\tick},
	scaled y ticks=false
]
...
\end{axis}
\end{tikzpicture}
\caption{Mean aspect ratios - $AR_{mean}$ - of triangles from output meshes from all test cases. Note that the \textquotesingle{}BCDL Unmerged\textquotesingle{} and \textquotesingle{}Simple Unmerged\textquotesingle{} results for the sphere mesh are much larger than all other results. See section \ref{appendix-b} for numerical values.}
\end{figure}
\FloatBarrier
\begin{multicols}{2}
...
Additionally, it is important to compare the effect of lattice structure on these results. The body-centered lattice was on average, $124.82\%$ more expensive to perform the vertex pass on than the simple-cubic arrangement, in terms of time.

However, it is critical to note that the body-centered lattice contains twice as many sample points which must be evaluated. The body-centered arrangement was also $48.67\%$ more expensive to compute the geometry pass for than the simple-cubic arrangement.

The tetrahedron arrangement seemed to have little impact on the fraction of triangles discarded during clustering. However, the body-centered arrangement did produce $38.96\%$ more triangles overall after merging was applied, compared to the same sample object processed using the simple-cubic lattice.
...
The results also suggest that while integrated clustering method may not produce triangles as uniformly as the post-processed method, the overall triangle count which results is lower. Further in the integrated method's favour is the topology preservation it provides, not to mention the huge performance benefit over the post-processed technique, making it an all-round more effective method for reducing triangle counts in runtime-generated procedural meshes.

\columnbreak

\subsection{Questionnaire}\label{questionnaire-results}

The questionnaire was conducted on a sample size of 18 volunteer participants. The raw results can be found in Appendix D (section \ref{appendix-d}). By summing preference factors for identical test configurations (see Fig. \ref{fig:preftab}), an overall preference weight for each test characteristic can be derived (see Fig. \ref{fig:prefoverall}).

This reveals that overall, the most visually appealing set of test parameters was the integrated merging using the simple cubic tetrahedral arrangement configuration. This subverts the expectation that the body-centered tetrahedral arrangement guarantees better shading appearance.

However, it is interesting to note that, on average, the body-centered tetrahedral arrangement is preferred over simple cubic, which could imply that the sample size is insufficient, or that the choice and limited range of sample objects is too narrow to effectively evaluate the overall visual performance of the two arrangements.

With regard to clustering method, there also exists a significant preference toward the integrated technique, which matches the predictions of the literature.
...
[ ] I understand that no personally identifying information is stored with my response, and that this means my response cannot be withdrawn after it is submitted
